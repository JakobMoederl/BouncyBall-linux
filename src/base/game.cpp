...
int i, j; //global loop variables (what are they deed for again?)
...
//destructor (you don't say, what exactly was my train of though writing that comment years ago?)
Game::~Game(void) {
}
...
bool Game::initFirst() {
    ...
    snprintf(lvlFile, 32, "lvl/%i.lvl", 1);
    ...
    if (!this->lvl[0].loadLvl(lvlFileStream)) {
        fprintf(stderr, "Couldn't read level file '%s'\n", lvlFile);
        return false;
    }
    lvlFileStream.close();

    snprintf(lvlFile, 32, "lvl/%i.lvl", 2);
    lvlFileStream.open(lvlFile, ios::in);
    if (lvlFileStream.is_open()) {
...
bool Game::init(int lvlNumber, Level &lvl) {
    ...
    snprintf(lvlFile, 32, "lvl/%i.lvl", lvlNumber);
    lvlFileStream.open(lvlFile, ios::in);
    if (!lvlFileStream.is_open()) {
        return false;
    }
    if (!lvl.loadLvl(lvlFileStream)) {
        fprintf(stderr, "couldnt read lvl File '%s'\n", lvlFile);
        return false;
    }
...
//game main routine, does everything
void Game::playFrame(GLfloat time_difference) {        //game main loop (does everything)
    ...
    if (!pause) {
        ...
        if (won && counter > 2.5) {
            ...
            if (counter > DEATH_ANIMATION_TIME + 0.2f ||
                getLvl().getPlayer().getY() - getLvl().getPlayer().getRadius() < 0) {
                resetLvl();
            } else {
...
// Calculates if there are any collisions
void Game::calcCollisions() {
    ...
    for (i = 0; i < getLvl().getSpikeBlockAnz(); i++) {
        if (getLvl().getSpikeBlock(i).getKollisionEdge(getPlayer()) ||
            getLvl().getSpikeBlock(i).getKollisionEdge(getPlayer())) {
...
//init the current lvl new (so you start from the beginning)
void Game::loose(void) {
    if (won) { //when the player would die after he already won the lvl
        return;
    }
...
//function should be called when you won the lvl
//inits the next lvl
void Game::win(void) {
    counter = 0.0f;
...
//draws the game (everything from scrolling, objects, player)
void Game::draw(void) {
    //reset display
    ...
    if (won) {
        glTranslatef(-x_lvl_won + (x_lvl_won - getLvl(activeLvl + 1).getStartX()) * counter / 2.5f,
                     -y_lvl_won + (y_lvl_won - getLvl(activeLvl + 1).getStartY()) * counter / 2.5f,
                     4.0f * counter / 2.5f);
...
void Game::drawInterface() {
    ...
    print("Smileys: %i/%i", -0.4f * (float) frame_width / (float) frame_height, 0.315f,
          (getLvl().getGoodSmileyAnzahl() - getLvl().getActiveSmileys()), getLvl().getGoodSmileyAnzahl());
...
void Game::drawBackground() {
    ...
    glBegin(GL_QUADS);
    glTexCoord2f(0, getLvl().getBackgroundFaktorY());
    glVertex3f(0, (float) getLvl().getHeight(), -16.0f);
    glTexCoord2f(getLvl().getBackgroundFaktorX(), getLvl().getBackgroundFaktorY());
    glVertex3f((float) getLvl().getWidth(), (float) getLvl().getHeight(), -16.0f);
    glTexCoord2f(getLvl().getBackgroundFaktorX(), 0);
    glVertex3f((float) getLvl().getWidth(), 0, -16.0f);
    glTexCoord2f(0, 0);
...
void Game::drawNextLvlBackground() {
    ...
    glBegin(GL_QUADS);
    glTexCoord2f(0, getLvl(activeLvl + 1).getBackgroundFaktorY());
    glVertex3f(0, (float) getLvl(activeLvl + 1).getHeight(), -16.0f);
    glTexCoord2f(getLvl(activeLvl + 1).getBackgroundFaktorX(), getLvl(activeLvl + 1).getBackgroundFaktorY());
    glVertex3f((float) getLvl(activeLvl + 1).getWidth(), (float) getLvl(activeLvl + 1).getHeight(), -16.0f);
    glTexCoord2f(getLvl(activeLvl + 1).getBackgroundFaktorX(), 0);
    glVertex3f((float) getLvl(activeLvl + 1).getWidth(), 0, -16.0f);
    glTexCoord2f(0, 0);
    glVertex3f(0, 0, -16.0f);
    glEnd();
    glTranslatef(0.0f, 0.0f, 4.0f);
